...
def _set_cache_headers(response: Response, *, cache_hit: bool) -> None:
    ...
    response.headers["X-MRIQC-Cache"] = "HIT" if cache_hit else "MISS"


def _cached_payload(
    response: Response,
    cache: TimedCache[object],
    key: tuple[object, ...],
    factory: Callable[[], object],
) -> object:
    cache_hit, payload = cache.get_or_set(key, factory)
    _set_cache_headers(response, cache_hit=cache_hit)
    return payload
...
def create_app(*, database_url: str | None = None) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        yield
        if get_response_cache.cache_info().currsize:
    ...
    @lru_cache
    def get_response_cache() -> TimedCache[object]:
        return TimedCache(
    ...
    def list_modalities(
        response: Response,
        cache: TimedCache[object] = Depends(get_response_cache),
    ) -> dict[str, object]:
    ...
    def overview(
        ...
        return _cached_payload(
            ...
            cache,
            ("overview", filters),
            lambda: profiler.overview(filters=filters),
        )
        ...
        try:
            return _cached_payload(
                ...
                (
                    ...
                    duplicate_member_limit,
                    extra_key_limit,
                    filters,
                ),
                lambda: profiler.modality_profile(
                    modality,
                    view=view,
                    filters=filters,
                    top_n=top_n,
    ...
    def modality_missingness(
        ...
        try:
            return _cached_payload(
                response,
                cache,
                ("missingness", modality, view, filters),
                lambda: {
                    "modality": modality,
                    "view": view.value,
    ...
    def modality_distribution(
        ...
        try:
            return _cached_payload(
                response,
                cache,
                ("distribution", modality, field_name, view, limit, filters),
                lambda: {
                    "modality": modality,
                    "field": field_name,
                    "view": view.value,
    ...
    def modality_extra_keys(
        ...
        try:
            return _cached_payload(
                response,
                cache,
                ("extra-keys", modality, column_name, view, limit, filters),
                lambda: {
                    "modality": modality,
                    "view": view.value,
                    "filters": filters.to_dict(),
    ...
    def modality_metric_summaries(
        ...
        try:
            return _cached_payload(
                response,
                cache,
                ("metric-summaries", modality, view, filters),
                lambda: {
                    "modality": modality,
                    "view": view.value,
                    "filters": filters.to_dict(),
    ...
    def modality_metric_distribution(
        ...
        try:
            return _cached_payload(
                response,
                cache,
                ("metric-distribution", modality, field_name, view, bins, filters),
                lambda: {
                    "modality": modality,
                    "field": field_name,
                    "view": view.value,
